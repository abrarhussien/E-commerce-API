...
const { findAllproducts, findProductById, createNewProduct, findProductByName, UpdateProductService, deleteOneProductService } = require('../services/product.service')
...
const addProduct = async (req, res) => {
    ...
    try{
        ...
        if(!category){return res.status(404).send("wrong category");}

        const product =await createNewProduct(value.productName, category["_id"], value.description, value.price,  value.quantity  )
        res.send(product)
    }catch(error){
...
const updateProduct = async (req, res) => {
    ...
    try{
        const product =await  findProductById(id);
        if(!product){return res.status(404).send("incorrect id")}
...
const deleteProduct = async(req , res) => {
    ...
    try{
    const product= await findProductById( id );
    if(!product){return res.status(404).send("Not found or Already deleted")}
